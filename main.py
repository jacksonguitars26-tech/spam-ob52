...
# Optimized for fast requests - NO SLEEP DELAYS
import requests
import os
import psutil
import sys
            ...
            try:
                ...
                while True:
                    try:
                        ...
                        if not self.DaTa:
                            break  # NO SLEEP - immediate break
                    except (
    ...
    async def ChaT(self, Token, tok, host, port, key, iv, bot_uid, R):
        ...
        while True:
            try:
                ...
                # NO SLEEP - immediate next
                self.writer.write(GLobaL(T, key, iv))
                await self.writer.drain()
                # NO SLEEP - immediate set ready
                R.set()
                while True:
                    try:
                        ...
                        if self.DaTa.hex().startswith("1200") and b"SecretCode" in self.DaTa:
                            ...
                            # Send invite immediately (NO SLEEP)
                            try:
                            ...
                            # Quit chat immediately (NO SLEEP)
                            try:
                            ...
                            # NO SLEEP - print and continue immediately
...
def load_accounts(file_path="sb.json"):
    """sb.json থেকে অ্যাকাউন্ট লোড করে"""
    import os
...
def StarT_SerVer():
    print(render("Ax Romjan ", colors=["white", "yellow"], align="center"))
    TexT = f"[TarGeT InFo] > BoTs arE OnLine\n[BoT sTaTus] > [bold green]ConEcTed SuccEssFuLy[/bold green]"
    panel = Panel(
        Align.center(TexT),
        title="[bold yellow]FF - Ax Romjan [/bold yellow]",
    ...
    forced_target_input = input("Enter target UID to force (IF YOU ARE BAD _-_-_Ax Romjan  IS YOUR DAD🥶): ").strip()
